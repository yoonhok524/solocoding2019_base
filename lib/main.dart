...
import 'package:solocoding2019_base/forecast.dart';
import 'package:solocoding2019_base/weather.dart';
...
class MyAppState extends State<MyApp> {
  ...
  _forecastData(ForecastResp forecastResp) {
    return Row(
      ...
      children: forecastResp.list.skip(1).map((i) => _buildForecast(i)).toList(),
    );
  }
...
Future<WeatherResp> _fetchWeather() async {
  ...
  final response =
      await http.get("$BASE_URL/weather?APPID=$API_KEY&lat=$lat&lon=$lon");
...
Future<ForecastResp> _fetchForecast(double lon, double lat) async {
  final response = await http
      .get("$BASE_URL/forecast/daily?APPID=$API_KEY&lat=$lat&lon=$lon&cnt=4");
  if (response.statusCode == 200) {
